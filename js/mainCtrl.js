angular.module('itunes').controller('mainCtrl', function($scope, itunesService, $filter){
  ...
  $scope.gridOptions = { 
      ...
      columnDefs: [
        ...
        {field: 'trackName', displayName: 'Song', enableFiltering: true, filter: {term: $scope.filterTerm}},
        ...
        {field: 'trackPrice', displayName: 'Single Price'},
...
$scope.getSongData = function(searchTerm) {
  itunesService.getSongData(searchTerm).then(function(response) {
    $scope.songData = response;
  });
};

// $scope.refreshGrid = function(filterTerm) {
//   $scope.gridOptions.data = $filter('filter')($scope.songData, filterTerm);
// };
